...
app=FastAPI(title='WaterPulse AI Agent API', version='10.0')
...
@app.get('/api/health')
def health():
    return {'status':'ok','app':'WaterPulse AI Agent','version':'10.0','deployment_layout':'server-self-healing','deepseek_configured':deepseek_configured(),'deepseek_model':deepseek_model(),'field_contract':contract_summary(),'timestamp':now_iso()}
...
def _reference_fallback(message: str, ref: dict[str,Any], intent: str="general") -> str:
    ...
    if material:
        return (f"我可以先分析 {material} 的上游水风险机制和管理重点，不需要先上传企业数据。"
...
def _auto_analyze(sid: str, sess: dict[str, Any], *, requested_scenario: str|None=None,
    ...
    if not sess.get('records'):
        return {'status':'needs_input','message':'可以继续做更贴近你企业实际采购情况的分析。为了判断哪些供应来源最值得关注，我还需要知道原材料来自哪些地区或供应商，以及各自大概占多少。你可以上传采购/ESG结构化数据（Excel/CSV），也可以下载 Excel 模板填写后上传；如果信息不多，也可以直接在对话框里输入。',
    ...
    if requested_scenario:
        ...
        if sres.get('data') is not None:
            ...
            actions=[_action('查看当前分析结果','show_current'),_action('导出企业决策报告','export_report')]
    ...
    status='success' if meta.get('complete',True) else 'partial'
    actions=[_action('模拟主要供应地中断','run_scenario',scenario_type='NodeFailure'),
             _action('导出企业决策报告','export_report'),
...
def agent_status():
    return {'status':'ok','agent_version':'10.0-demo-interaction-final',
...
async def agent_message(
            ...
            if res.get('kind')=='table':
                ...
                if norm.empty:
                    return {'session_id':sid,'status':'needs_input',
                            ...
                            'actions':[_action('上传采购/ESG数据','focus_upload'),_action('下载 Excel 模板','download_template')],
                            'trace':trace,'ai':_ai_meta()}
                ...
                return {'session_id':sid,'status':'needs_confirmation',
                        'message':'我从非标准文件中提取到了以下供应链候选信息。候选信息需要你确认后才能进入正式计算：\n'+'\n'.join('• '+x for x in preview),
    ...
    if action=='show_reference_locations':
        ...
        if not ref.get("material_supported"):
            ...
            return {'session_id':sid,'status':'partial','message':text,
                    'actions':[_action('上传企业数据','focus_upload')],'trace':trace,'ai':_ai_meta()}
    ...
    if action=='retry_explanation':
        ...
        actions=[_action('导出企业决策报告','export_report')]
        if not meta.get('complete',True): actions.append(_action('重试解释','retry_explanation'))
    ...
    if action=='scenario_menu':
        return {'session_id':sid,'status':'success',
                'message':'当前 MVP 的主验收压力测试是“主要供应地中断”。你可以先调整中断比例，观察供应损失、库存缓冲、替代供应和未满足需求。',
                'actions':[_action('运行主要供应地中断','run_scenario',scenario_type='NodeFailure'),
                           _action('导出企业决策报告','export_report'),
                           _action('下载本次运行记录','download_run_record')],
                'trace':trace,'ai':_ai_meta()}
    ...
    if msg and not sess.get('records') and intent=='analyze':
        ...
        if not cand.empty:
            ...
            return {'session_id':sid,'status':'needs_confirmation',
                    ...
                    'actions':[_action('确认并继续分析','confirm_mapping'),_action('上传采购/ESG数据','focus_upload')],'trace':trace,'ai':_ai_meta(),
    ...
    return {'session_id':sid,'status':'needs_input',
            'message':'你好，我是 WaterPulse。没有文件也可以直接问我：某种原材料有哪些上游水风险、某个供应地区要关注什么、怎样降低风险，或者某种压力情况下可能发生什么。我会先用 AI 回答和判断；如果需要更进一步的量化结果，再根据你的需求选择项目参考数据、企业资料或 Excel 模板。',
